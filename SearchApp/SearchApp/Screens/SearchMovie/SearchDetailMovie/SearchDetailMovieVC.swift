...
class SearchDetailMovieVC: UIViewController {
    ...
    private var movieDetail: SearchList?
    
    var searchDetailViewModel: SearchDetailMovieViewModelProtocol?
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = movieDetail?.artworkUrl100
        favorite.name = movieDetail?.trackName
        favorite.price = movieDetail?.trackPrice ?? 0.0
        
        appDelegate.saveContext()
    }
}

extension SearchDetailMovieVC: SearchDetailMovieViewControllerProtocol {
    func handleOutPut(_ output: SearchDetailMovieViewModelOutPut) {
        switch output {
        case .selectSearchMovieItem(let movieDetailData):
            self.movieDetail = movieDetailData
            propertyUI(searchObject: movieDetailData)
