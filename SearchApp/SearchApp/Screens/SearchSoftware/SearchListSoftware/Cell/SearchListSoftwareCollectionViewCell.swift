//
//  SearchListSoftwareCollectionViewCell.swift
//  SearchApp
//
...
class SearchListSoftwareCollectionViewCell: UICollectionViewCell {
    ...
    private func propertyUI(item: SearchList) {
        guard let name = item.trackName, let image = item.artworkUrl100 else { return }
        searchName.text = "Name: \(name)"
        setImage(value: image)
        
        if item.wrapperType != "software" {
            searchPrice.isHidden = false
            guard let price  = item.trackPrice  else { return }
...
extension  SearchListSoftwareCollectionViewCell {
