...
class SearchDetailSoftwareVC: UIViewController {
    ...
    private var softwareDetail: SearchList?
    
    var searchDetailViewModel: SearchDetailSoftwareViewModelProtocol?
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = softwareDetail?.artworkUrl100
        favorite.name = softwareDetail?.trackName
        favorite.price = softwareDetail?.trackPrice ?? 0.0
        
        appDelegate.saveContext()
    }
}

extension SearchDetailSoftwareVC: SearchDetailSoftwareViewControllerProtocol {
    func handleOutPut(_ output: SearchDetailSoftwareViewModelOutPut) {
        switch output {
        case .selectSearchSoftwareItem(let softwareDetailData):
            self.softwareDetail = softwareDetailData
            propertyUI(searchObject: softwareDetailData)
