...
class SearchDetailEbookVC: UIViewController {
    ...
    private var ebookDetail: SearchList?
    
    var searchDetailViewModel: SearchDetailEbookViewModelProtocol?
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = ebookDetail?.artworkUrl100
        favorite.name = ebookDetail?.trackName
        favorite.price = ebookDetail?.trackPrice ?? 0.0
        
        appDelegate.saveContext()
    }
}

extension SearchDetailEbookVC: SearchDetailEbookViewControllerProtocol {
    func handleOutPut(_ output: SearchDetailEbookViewModelOutPut) {
        switch output {
        case .selectSearchEbookItem(let ebookDetailData):
            self.ebookDetail = ebookDetailData
            propertyUI(searchObject: ebookDetailData)
