...
class SearchDetailPodcastVC: UIViewController {
    ...
    private var podcastDetail: SearchList?
    private let context = appDelegate.persistentContainer.viewContext
    
    var searchDetailViewModel: SearchDetailPodcastViewModelProtocol?
    ...
    @objc func favorite(_ favoriteButton: UIButton) {
        ...
        favorite.image = podcastDetail?.artworkUrl100
...
extension SearchDetailPodcastVC: SearchDetailPodcastViewControllerProtocol {
    func handleOutPut(_ output: SearchDetailPodcastViewModelOutPut) {
        switch output {
        case .selectSearchPodcastItem(let podcastDetailData):
            self.podcastDetail = podcastDetailData
            propertyUI(searchObject: podcastDetailData)
