...
class SearchListPodcastVC: UIViewController {
    ...
    private var searchCollectionProvider = ProductListProvider()
    
    
    var searchListViewModel: SearchListPodcastViewModelProtocol?
...
extension SearchListPodcastVC: UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        guard let text = searchBar.text else { return }
        
        searchListViewModel?.loadSearchPodcast(term: text)
    }
    
    func searchBarCancelButtonClicked(_ searchBar: UISearchBar) {
        searchListViewModel?.loadSearchPodcast(term: "")
    }
}

extension SearchListPodcastVC: SearchListPodcastViewModelDelegate {
    func handleOutPut(_ output: SearchListPodcastViewModelOutPut) {
        switch output {
        case .showSearchListPodcast(let searchData):
            self.searchItem = searchData
            searchCollectionProvider.loadProductList(item: searchData)
            refreshCollection()
...
extension SearchListPodcastVC: SearchListPodcastProviderDelegate {
    func selected(at select: SearchList) {
        let viewController = SearchDetailPodcastBuilder.make(searchObject: select)
        self.show(viewController, sender: nil)
    }
}

extension SearchListPodcastVC {
